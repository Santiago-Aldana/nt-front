...
import { obtenerCookie, borrarCookiesSesion } from "../../../lib/cookies";
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-14 bg-trenza-indigo flex items-center justify-between px-4 z-20">
      ...
      <div className="relative" ref={menuRef}>
        ...
        >
          {urlAvatar ? (
            <img src={urlAvatar} alt="Foto de perfil" className="w-full h-full object-cover" />
          ) : (
            <span className="text-xs font-medium text-trenza-crema">{iniciales}</span>
          )}
        </button>
  ...
  <div className="fixed inset-0 z-30 flex items-center justify-center pointer-events-none">
    ...
    >
      <button
        ...
        aria-label="Cerrar"
      >
        <X size={18} className="text-trenza-texto/60" />
      </button>
      ...
      <div className="flex flex-col items-center -mt-2">
        ...
        <p className="text-lg font-medium text-trenza-texto mt-3">{usuario.username}</p>
